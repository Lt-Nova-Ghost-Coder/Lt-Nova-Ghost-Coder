<h1 align="center">Krishna Saini</h1>
<h3 align="center">Student @ IIIT-DELHI | Software Engineer | Part-time Gamer | Sometimes a Cook</h3>

<br>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Lt-Nova-Ghost-Coder&label=Profile%20views&color=0e75b6&style=flat" />
</p>

<br>

<p align="center">
  <img src="https://github.com/user-attachments/assets/3ac6ba09-cf99-4acb-be93-dc105169b3f4" style="width: 45%; display: inline-block;" />
  <img src="https://github.com/user-attachments/assets/6d89159a-6f76-4b6b-9a12-567bd779c8be" style="width: 45%; display: inline-block;" />
</p>

<br>

<h3 align="center">My Badegs Badges</h3>

<p align="center">
    <img src="https://github.com/Lt-Nova-Ghost-Coder/Lt-Nova-Ghost-Coder/blob/main/HactoberfestContributor.webp" height="100" />
  &nbsp;&nbsp;&nbsp;&nbsp;
    <img src="https://github.com/Lt-Nova-Ghost-Coder/Lt-Nova-Ghost-Coder/blob/main/HolopinXHacktoberfest.webp" height="100" />
  &nbsp;&nbsp;&nbsp;&nbsp;
    <img src="https://github.com/Lt-Nova-Ghost-Coder/Lt-Nova-Ghost-Coder/blob/main/NSOCContributor.png" height="100" />
  &nbsp;&nbsp;&nbsp;&nbsp;
    <img src="https://github.com/Lt-Nova-Ghost-Coder/Lt-Nova-Ghost-Coder/blob/main/NSOCParticipationBadge.png" height="100" /> 
  &nbsp;&nbsp;&nbsp;&nbsp;
    <img src="https://github.com/Lt-Nova-Ghost-Coder/Lt-Nova-Ghost-Coder/blob/main/contributor.png" height="100" />
  &nbsp;&nbsp;&nbsp;&nbsp;
    <img src="https://github.com/Lt-Nova-Ghost-Coder/Lt-Nova-Ghost-Coder/blob/main/open.png" height="100" />
  &nbsp;&nbsp;&nbsp;&nbsp;

<br>

<p>
<h3>About me:</h3>

Second-year full-time CS student, part-time software enginner. Working on projects, diving into DSA, and figuring out how things really work under the hood. Outside coding, I’m a video game geek—so whether it’s debugging code or beating a boss level,<br>

<b>I’m always up for a challenge!</b>.

</p>

<br>

<h3>Projects</h3>

<div>

  <h3>🔹 Veritas – AI Video Detection</h3>
  <p>
    Full-stack tool for AI detection using frame-level analysis, interactive dashboard, and downloadable forensic reports.<br>
    <b>Tech:</b> React, TypeScript, FastAPI, OpenCV, PyTorch
    <br>
    <a href = "https://github.com/Lt-Nova-Ghost-Coder/AI-Video-Detection-tool">Click Here</a>
  </p>

  <br>

  <h3>🔹 Full Stack Gym & Sports System</h3>
  <p>
    Web application for gym access and equipment lending with authentication, role-based dashboards, and inventory management.<br>
    <b>Tech:</b> Next.js, TypeScript, MySQL, Node.js
    <br>
    <a href = "https://github.com/Lt-Nova-Ghost-Coder/DBMS_SEMESTER_PROJECT">Click Here</a>
  </p>

  <br>

  <h3>🔹 College ERP System</h3>
  <p>
    University ERP platform with role-based access, academic workflows, and automated grading system.<br>
    <b>Tech:</b> Java, Swing, MySQL
    <br>
    <a href = "https://github.com/Lt-Nova-Ghost-Coder/Erp_2025">Click Here</a>
  </p>

  <br>

  <h3>🔹 Simple Scheduler</h3>
  <p>
    C-based scheduler integrated with a shell, implementing round-robin scheduling with fixed time slices.<br>
    <b>Tech:</b> C, Operating Systems
    <br>
    <a href = "https://github.com/Lt-Nova-Ghost-Coder/simple-unix-scheduler">Click Here</a>
  </p>

  <br>

  <h3>🔹 Simple Shell</h3>
  <p>
    Custom shell built in C to execute user commands and simulate OS-level command handling.<br>
    <b>Tech:</b> C, Operating Systems
    <br>
    <a href = "https://github.com/Lt-Nova-Ghost-Coder/simple-unix-shell">Click Here</a>
  </p>

  <br>

  <h3>🔹 Live Resume Website</h3>
  <p>
    Responsive personal portfolio with modern UI and smooth animations.<br>
    <b>Tech:</b> React, TypeScript, HTML, CSS
    <br>
    <a href = "https://github.com/Lt-Nova-Ghost-Coder/resume">Click Here</a>
  </p>

</div>

<br>

<h3>Programming Languages and Tools</h3>

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=py,c,cpp,java,html,linux,mysql,vscode,pycharm,ubuntu,vim,windows,figma,github,idea" />
  </a>
</p>

<br>


<h3>Connect with me:</h3>

<p align="center">
  <a href="https://linkedin.com/in/krishna-saini-04a309304">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" height="50" />
  </a>
  &nbsp;&nbsp;&nbsp;&nbsp;
  <a href="https://www.instagram.com/k.r.i.s.h_ks.co/">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/instagram.svg" height="50" />
  </a>
  &nbsp;&nbsp;&nbsp;&nbsp;
  <a href="https://leetcode.com/u/kksks1234/">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/leet-code.svg" height="50" />
  </a>
</p>

<br>


<b>My Game-Count: 40 </b><br>
Tell me yours too!

